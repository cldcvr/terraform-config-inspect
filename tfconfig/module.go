...
type Module struct {
	...
	Inputs map[string]map[string][]AttributeReference `json:"inputs"`
...
func (m Module) GetResourceAttributeReferences(varName string) []RelativeAttributeReference {
	...
	for _, mc := range m.ModuleCalls {
		if mc.Module != nil {
			for inpName, inp := range mc.Inputs {
				if inp.Type() == "var" && inp.Name() == varName {
					found := mc.Module.GetResourceAttributeReferences(inpName)
					for i := range found {
						found[i].RelativePath = append(found[i].RelativePath, inp.PathSegments()...)
					}
	...
	for _, res := range m.ManagedResources {
		for inpName, inp := range res.Inputs {
			if inp.Type() == "var" && inp.Name() == varName {
				references = append(references, ResourceAttributeReference{
					Module:        &m,
					ResourceType:  res.Type,
					ResourceName:  res.Name,
					AttributePath: []string{inpName},
				}.MakeRelative(inp.PathSegments()))
			}
		}
	}
	return references
...
func (r RelativeAttributeReference) PathSegments() []string {
	return append(append([]string{}, r.AttributePath...), r.RelativePath...)
}
...
func (r ResourceAttributeReference) Path() string {
	return buildAttributePath(r.PathSegments()...)
}

func (r ResourceAttributeReference) PathSegments() []string {
	return append([]string{}, r.AttributePath...)
}
